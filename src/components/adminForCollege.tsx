...
const AdminForCollege = () => {
  ...
  const getLandUseData = (college) => {
    if (!college.landUse) return [];
    
    return [
      { name: 'Building Area', value: college.landUse.buildingArea || 0, fill: LAND_USE_COLORS['Building Area'] },
      { name: 'Playground Area', value: college.landUse.playgroundArea || 0, fill: LAND_USE_COLORS['Playground Area'] },
      { name: 'Natural Forest Area', value: college.landUse.naturalForestArea || 0, fill: LAND_USE_COLORS['Natural Forest Area'] },
      { name: 'Plantation Area', value: college.landUse.plantationArea || 0, fill: LAND_USE_COLORS['Plantation Area'] },
      { name: 'Leased Area', value: college.landUse.leasedArea || 0, fill: LAND_USE_COLORS['Leased Area'] }
    ].filter(item => item.value > 0);
  };

  const getLandOwnershipData = (college) => {
    if (!college.landOwnership) return [];
    
    const ownership = college.landOwnership;
    return [
  ...
  const getTraditionalUnitsData = (college) => {
    if (!college.landArea?.traditionalUnits) return [];
    
    const units = college.landArea.traditionalUnits;
    return [
  ...
  const getLandAreaMetrics = (college) => {
    if (!college.landArea) return null;
    
    return {
      squareMeters: college.landArea.squareMeters || 0,
      acquisitionDate: college.landArea.acquisitionDate ? new Date(college.landArea.acquisitionDate).toLocaleDateString() : 'N/A',
      taxClearanceStatus: college.landArea.taxClearanceStatus || 'Not Specified',
      haalsabikStatus: college.landArea.haalsabikStatus || 'Not Specified'
    };
  };
  ...
  const renderLandDataSection = (college) => {
    ...
    console.log(college,"college data value data")
    ...
    const landMetrics = getLandAreaMetrics(college.infrastructure);
    const landUseData = getLandUseData(college.infrastructure);
    const traditionalUnitsData = getTraditionalUnitsData(college.infrastructure);
    const ownershipData = getLandOwnershipData(college.infrastructure);
                        ...
                        <>
                          <ListItem>
                            <ListItemText 
                              ...
                              secondary={`Rs. ${(college.landUse.leaseIncome || 0).toLocaleString()}`}
                            />
                          </ListItem>
                          <ListItem>
                            <ListItemText 
                              ...
                              secondary={
                                <Chip 
                                  label={college.landUse.encroachmentExists ? 'Yes' : 'No'} 
                                  color={college.landUse.encroachmentExists ? 'error' : 'success'}
                                  size="small"
                                />
                              }
                            />
                          </ListItem>
                          {college.landUse.encroachmentExists && (
                            <ListItem>
                              <ListItemText 
                                primary="Encroachment Details" 
                                secondary={college.landUse.encroachmentDetails || 'No details provided'}
                              />
                            </ListItem>
                          )}
                          <ListItem>
                            <ListItemText 
                              ...
                              secondary={
                                <Chip 
                                  label={college.landUse.masterPlanExists ? 'Available' : 'Not Available'} 
                                  color={college.landUse.masterPlanExists ? 'success' : 'default'}
                                  size="small"
                                />
                              }
                            />
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    {college.landUse.protectionSteps || 'No protection steps specified'}
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    {college.landUse.commercialUseSuggestions || 'No suggestions provided'}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    {college.landUse.commercialPlans || 'No commercial plans specified'}
                  </Typography>
                  ...
                  <Typography variant="body2">
                    {college.landUse.suggestions || 'No additional suggestions'}
                  </Typography>
                </Grid>
              </Grid>
            </CardContent>
  ...
  // Enhanced export functions to include land data
  const exportToPDF = () => {
    if (!selectedCollege) return;
    ...
    // Land Data Section
    if (selectedCollege.landArea || selectedCollege.landOwnership || selectedCollege.landUse) {
      doc.addPage();
      doc.setFontSize(16);
      doc.text('Land & Property Information', 20, 20);
      
      const landData = [];
      if (selectedCollege.landArea) {
        landData.push(
          ['Land Area (m²)', (selectedCollege.landArea.squareMeters || 0).toString()],
          ['Acquisition Date', selectedCollege.landArea.acquisitionDate ? new Date(selectedCollege.landArea.acquisitionDate).toLocaleDateString() : 'N/A'],
          ['Tax Clearance Status', selectedCollege.landArea.taxClearanceStatus || 'N/A'],
          ['Haalsabik Status', selectedCollege.landArea.haalsabikStatus || 'N/A']
        );
      }

      if (selectedCollege.landUse) {
        landData.push(
          ['Building Area', (selectedCollege.landUse.buildingArea || 0).toString()],
          ['Playground Area', (selectedCollege.landUse.playgroundArea || 0).toString()],
          ['Natural Forest Area', (selectedCollege.landUse.naturalForestArea || 0).toString()],
          ['Plantation Area', (selectedCollege.landUse.plantationArea || 0).toString()],
          ['Leased Area', (selectedCollege.landUse.leasedArea || 0).toString()],
          ['Lease Income', `Rs. ${(selectedCollege.landUse.leaseIncome || 0).toLocaleString()}`],
          ['Encroachment Exists', selectedCollege.landUse.encroachmentExists ? 'Yes' : 'No'],
          ['Master Plan Exists', selectedCollege.landUse.masterPlanExists ? 'Yes' : 'No']
        );
      }
      ...
      if (selectedCollege.landOwnership) {
        const ownershipData = [
          ['Lalpurja', selectedCollege.landOwnership.lalpurja?.area || 'N/A', selectedCollege.landOwnership.lalpurja?.address || 'N/A'],
          ['Bhogadhikar', selectedCollege.landOwnership.bhogadhikar?.area || 'N/A', selectedCollege.landOwnership.bhogadhikar?.address || 'N/A'],
          ['Local Government', selectedCollege.landOwnership.localGovernment?.area || 'N/A', selectedCollege.landOwnership.localGovernment?.address || 'N/A'],
          ['Other', selectedCollege.landOwnership.other?.area || 'N/A', selectedCollege.landOwnership.other?.address || 'N/A']
        ];
  ...
  const exportToExcel = () => {
    ...
    if (selectedCollege.landArea || selectedCollege.landOwnership || selectedCollege.landUse) {
      const landData = [
        ...
        ['Land Area Details', '', ''],
        ['Square Meters', selectedCollege.landArea?.squareMeters || 0],
        ['Acquisition Date', selectedCollege.landArea?.acquisitionDate ? new Date(selectedCollege.landArea.acquisitionDate).toLocaleDateString() : 'N/A'],
        ['Tax Clearance Status', selectedCollege.landArea?.taxClearanceStatus || 'N/A'],
        ['Haalsabik Status', selectedCollege.landArea?.haalsabikStatus || 'N/A'],
        [''],
        ['Traditional Units', '', ''],
        ['Bigaha', selectedCollege.landArea?.traditionalUnits?.bigaha || 0],
        ['Katha', selectedCollege.landArea?.traditionalUnits?.katha || 0],
        ['Dhur', selectedCollege.landArea?.traditionalUnits?.dhur || 0],
        ['Ropani', selectedCollege.landArea?.traditionalUnits?.ropani || 0],
        ['Ana', selectedCollege.landArea?.traditionalUnits?.ana || 0],
        ['Daam', selectedCollege.landArea?.traditionalUnits?.daam || 0],
        ['Paisa', selectedCollege.landArea?.traditionalUnits?.paisa || 0],
        [''],
        ['Land Use Information', '', ''],
        ['Building Area', selectedCollege.landUse?.buildingArea || 0],
        ['Playground Area', selectedCollege.landUse?.playgroundArea || 0],
        ['Natural Forest Area', selectedCollege.landUse?.naturalForestArea || 0],
        ['Plantation Area', selectedCollege.landUse?.plantationArea || 0],
        ['Leased Area', selectedCollege.landUse?.leasedArea || 0],
        ['Lease Income', selectedCollege.landUse?.leaseIncome || 0],
        ['Encroachment Exists', selectedCollege.landUse?.encroachmentExists ? 'Yes' : 'No'],
        ['Master Plan Exists', selectedCollege.landUse?.masterPlanExists ? 'Yes' : 'No'],
        [''],
