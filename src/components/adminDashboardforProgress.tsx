...
//           startY: currentY,
//           head: [
//             ['Institution', 'Level', 'Program Name', 'Total', 'Male', 'Female', 'Scholarship', 'Rule Applied', 'New Adm', 'Graduated', 'Pass %', 'Approval']
//           ],
//           body: programData,
//           theme: 'grid',
...
//         'Institution', 'Level', 'Program Name', 'Program Total Students', 'Program Male Students', 'Program Female Students',
...
//   // NEW: Get student distribution by institution/faculty
//   const getInstitutionDistributionData = (report: ProgressReport) => {
//     if (!report.programs) return [];
    
//     const institutionMap = new Map();
    
//     report.programs.forEach(program => {
//       const institution = program.institution || 'Not Specified ';
//       const current = institutionMap.get(institution) || 0;
//       institutionMap.set(institution, current + (program.totalStudents || 0));
//     });
    
//     return Array.from(institutionMap, ([name, value]) => ({
//       name,
//       value,
//       percentage: ((value / report.totalStudents) * 100).toFixed(1)
//     }));
//   };

//   // NEW: Get student distribution by level
//   const getLevelDistributionData = (report: ProgressReport) => {
//     if (!report.programs) return [];
    
//     const levelMap = new Map();
    
//     report.programs.forEach(program => {
//       const level = program.level || 'Not Specified Level';
//       const current = levelMap.get(level) || 0;
//       levelMap.set(level, current + (program.totalStudents || 0));
//     });
    
//     return Array.from(levelMap, ([name, value]) => ({
//       name,
//       value,
//       percentage: ((value / report.totalStudents) * 100).toFixed(1)
//     }));
...
//   // NEW: Get program size comparison data
//   const getProgramSizeData = (report: ProgressReport) => {
//     if (!report.programs) return [];
    
//     return report.programs.map(program => ({
//       program: program.programName,
//       institution: program.institution || 'Unknown',
...
//   // NEW: Get institution-level combination data
//   const getInstitutionLevelData = (report: ProgressReport) => {
//     if (!report.programs) return [];
    
//     const combinationMap = new Map();
    
//     report.programs.forEach(program => {
//       const key = `${program.institution || 'Unknown'} - ${program.level || 'Unknown'}`;
//       const current = combinationMap.get(key) || 0;
//       combinationMap.set(key, current + (program.totalStudents || 0));
//     });
    
//     return Array.from(combinationMap, ([name, value]) => ({
//       name,
//       value
//     })).sort((a, b) => b.value - a.value);
//   };

//   // NEW: Overall Program Analysis Functions for All Colleges
//   const getAllInstitutionsData = () => {
//     const institutionMap = new Map();
...
//       name,
//       value,
//       percentage: ((value / getTotalStudents()) * 100).toFixed(1)
//     })).sort((a, b) => b.value - a.value);
//   };
...
//   const getTopProgramsData = () => {
//     const programMap = new Map();
    
//     reports.forEach(report => {
//       report.programs?.forEach(program => {
//         const programName = program.programName || 'Not Specified Program';
//         const current = programMap.get(programName) || 0;
//         programMap.set(programName, current + (program.totalStudents || 0));
//       });
//     });
    
//     return Array.from(programMap, ([name, value]) => ({
//       name,
//       value,
//       collegeCount: reports.filter(report => 
//         report.programs?.some(p => p.programName === name)
//       ).length
//     })).sort((a, b) => b.value - a.value).slice(0, 10);
//   };

//   const getCollegeProgramStats = () => {
//     return reports.map(report => ({
//       name: report.collegeName,
//       programs: report.programs?.length || 0,
//       students: report.totalStudents || 0,
//       avgStudentsPerProgram: report.programs?.length ? 
//         Math.round((report.totalStudents || 0) / report.programs.length) : 0
//     })).sort((a, b) => b.programs - a.programs);
//   };

//   const getInstitutionLevelCombinationData = () => {
//     const combinationMap = new Map();
...
//   // NEW: Student Distribution Analysis Functions
//   const getFacultyDistribution = () => {
//     const facultyMap = new Map();
    
//     reports.forEach(report => {
//       report.programs?.forEach(program => {
//         const faculty = program.institution || 'Not Specified ';
//         const current = facultyMap.get(faculty) || 0;
//         facultyMap.set(faculty, current + (program.totalStudents || 0));
//       });
//     });
    
//     return Array.from(facultyMap, ([name, value]) => ({
//       name,
//       students: value,
//       percentage: ((value / getTotalStudents()) * 100).toFixed(1)
...
//     return reports.reduce((sum, report) => sum + (report.totalStudents || 0), 0);
//   };

//   const getTotalPrograms = () => {
//     return reports.reduce((sum, report) => sum + (report.programs?.length || 0), 0);
//   };

//   const getAverageStudentsPerCollege = () => {
//     return reports.length ? Math.round(getTotalStudents() / reports.length) : 0;
//   };

//   const getAverageProgramsPerCollege = () => {
//     return reports.length ? Math.round(getTotalPrograms() / reports.length) : 0;
//   };
...
//     // NEW: Get the additional analysis data
//     const institutionDistributionData = getInstitutionDistributionData(selectedCollege);
//     const levelDistributionData = getLevelDistributionData(selectedCollege);
//     const programSizeData = getProgramSizeData(selectedCollege);
//     const institutionLevelData = getInstitutionLevelData(selectedCollege);
...
//                     Program-wise Student Data
//                   </CardTitle>
//                   <CardDescription>Detailed breakdown of students by institution, level and program</CardDescription>
//                 </CardHeader>
//                 <CardContent>
...
//                           <TableRow key={index}>
//                             <TableCell className="font-semibold">{program.institution}</TableCell>
//                             <TableCell>
...
//               {/* NEW: Program Analysis Section */}
//               <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
//                 <Card className="shadow-lg">
//                   <CardHeader>
//                     <CardTitle className="flex items-center text-xl">
//                       <Building className="h-5 w-5 mr-2 text-blue-600" />
//                       Student Distribution by Institution/Faculty
//                     </CardTitle>
//                     <CardDescription>Breakdown of students across different faculties and institutes</CardDescription>
//                   </CardHeader>
//                   <CardContent>
//                     <ResponsiveContainer width="100%" height={400}>
//                       <BarChart data={institutionDistributionData}>
//                         <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                         <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 12, fontWeight: 600 }} angle={-45} textAnchor="end" height={80} />
...
//                         />
//                         <Bar dataKey="value" fill="#3b82f6" name="Students" radius={[8, 8, 0, 0]} />
//                       </BarChart>
//                     </ResponsiveContainer>
//                   </CardContent>
//                 </Card>
...
//                     </CardTitle>
//                     <CardDescription>Breakdown of students across different academic levels</CardDescription>
//                   </CardHeader>
//                   <CardContent>
//                     <ResponsiveContainer width="100%" height={400}>
//                       <PieChart>
...
//                     </CardTitle>
//                     <CardDescription>Largest programs by student enrollment</CardDescription>
//                   </CardHeader>
//                   <CardContent>
//                     <ResponsiveContainer width="100%" height={400}>
//                       <BarChart 
//                         data={programSizeData.slice(0, 10)} 
//                         layout="vertical"
...
//                         <Tooltip 
//                           contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px' }}
//                           formatter={(value, name) => [value, name === 'totalStudents' ? 'Total Students' : name]}
//                         />
//                         <Legend />
//                         <Bar dataKey="totalStudents" fill="#8b5cf6" name="Total Students" radius={[0, 8, 8, 0]} />
//                         <Bar dataKey="maleStudents" fill="#3b82f6" name="Male Students" radius={[0, 8, 8, 0]} />
//                         <Bar dataKey="femaleStudents" fill="#ec4899" name="Female Students" radius={[0, 8, 8, 0]} />
//                       </BarChart>
//                     </ResponsiveContainer>
//                   </CardContent>
//                 </Card>
...
//                       Institution-Level Combination Analysis
//                     </CardTitle>
//                     <CardDescription>Student distribution across institution-level combinations</CardDescription>
//                   </CardHeader>
//                   <CardContent>
//                     <ResponsiveContainer width="100%" height={400}>
//                       <BarChart data={institutionLevelData}>
//                         <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                         <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} angle={-45} textAnchor="end" height={100} />
//                         <YAxis tick={{ fill: '#64748b', fontSize: 11 }} />
//                         <Tooltip 
//                           contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px' }}
//                           formatter={(value) => [value, 'Students']}
//                         />
//                         <Bar dataKey="value" fill="#f59e0b" name="Students" radius={[8, 8, 0, 0]} />
//                       </BarChart>
//                     </ResponsiveContainer>
//                   </CardContent>
//                 </Card>
...
//         {/* NEW: Student Distribution Charts Section */}
//         <div className="mb-8">
...
//               </CardTitle>
//               <CardDescription>
//                 Comprehensive breakdown of student enrollment across faculties, programs, and academic levels
//               </CardDescription>
//             </CardHeader>
//           </Card>
//         </div>
...
//           {/* Faculty/Institution Distribution */}
//           <Card className="shadow-lg">
//             <CardHeader>
//               <CardTitle className="flex items-center text-xl">
//                 <Building className="h-5 w-5 mr-2 text-blue-600" />
//                 Students by Faculty/Institution
//               </CardTitle>
//               <CardDescription>Total student distribution across different faculties and institutes</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <BarChart data={getFacultyDistribution()}>
//                   <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                   <XAxis 
//                     dataKey="name" 
//                     tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} 
...
//                     label={{ value: 'Number of Students', angle: -90, position: 'insideLeft', style: { fill: '#64748b', fontSize: 12 } }}
//                   />
//                   <Tooltip 
//                     contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px' }}
//                     formatter={(value, name) => [value, 'Students']}
...
//               </CardTitle>
//               <CardDescription>Most popular programs across all colleges by student count</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <BarChart 
...
//                     type="number" 
//                     tick={{ fill: '#64748b', fontSize: 11 }} 
//                     label={{ value: 'Number of Students', position: 'insideBottom', style: { fill: '#64748b', fontSize: 12 } }}
//                   />
//                   <YAxis 
//                     type="category" 
//                     dataKey="name" 
...
//                   <Tooltip 
//                     contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px' }}
//                     formatter={(value, name) => [value, name === 'students' ? 'Students' : 'Colleges']}
//                     labelFormatter={(label) => `Program: ${label}`}
//                   />
//                   <Bar dataKey="students" fill="#10b981" name="Total Students" radius={[0, 8, 8, 0]} />
//                 </BarChart>
//               </ResponsiveContainer>
//             </CardContent>
//           </Card>
...
//               </CardTitle>
//               <CardDescription>Distribution of students across different academic levels</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <PieChart>
...
//                 Enrollment Summary
//               </CardTitle>
//               <CardDescription>Overall student enrollment statistics</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <div className="space-y-6">
//                 <div className="grid grid-cols-2 gap-4">
//                   <div className="bg-blue-50 p-4 rounded-lg text-center">
//                     <div className="text-3xl font-bold text-blue-900">{getTotalStudents().toLocaleString()}</div>
//                     <div className="text-sm text-blue-700 font-semibold">Total Students</div>
//                   </div>
//                   <div className="bg-green-50 p-4 rounded-lg text-center">
//                     <div className="text-3xl font-bold text-green-900">{getTotalPrograms()}</div>
//                     <div className="text-sm text-green-700 font-semibold">Total Programs</div>
//                   </div>
//                 </div>
                
//                 <div className="bg-gray-50 p-4 rounded-lg">
//                   <h4 className="font-semibold text-gray-900 mb-3">Top 5 Faculties by Enrollment:</h4>
//                   <div className="space-y-2">
//                     {getFacultyDistribution().slice(0, 5).map((faculty, index) => (
//                       <div key={index} className="flex justify-between items-center py-2 border-b border-gray-200">
//                         <span className="text-sm font-medium text-gray-700">{faculty.name}</span>
//                         <div className="text-right">
//                           <span className="font-bold text-gray-900">{faculty.students.toLocaleString()}</span>
//                           <span className="text-xs text-gray-500 ml-2">({faculty.percentage}%)</span>
//                         </div>
//                       </div>
//                     ))}
//                   </div>
...
//                 <div className="bg-gray-50 p-4 rounded-lg">
//                   <h4 className="font-semibold text-gray-900 mb-3">Academic Level Distribution:</h4>
//                   <div className="space-y-2">
//                     {getLevelDistribution().map((level, index) => (
//                       <div key={index} className="flex justify-between items-center py-2 border-b border-gray-200">
//                         <span className="text-sm font-medium text-gray-700">{level.name}</span>
//                         <div className="text-right">
//                           <span className="font-bold text-gray-900">{level.students.toLocaleString()}</span>
...
//         {/* Existing Overall Program Analysis Section */}
//         <div className="mb-8">
//           <Card className="shadow-xl border-0">
//             <CardHeader>
//               <CardTitle className="text-2xl flex items-center">
//                 <Target className="h-6 w-6 mr-2 text-blue-600" />
//                 Overall Program Analysis - All Colleges
//               </CardTitle>
//               <CardDescription>
//                 Comprehensive analysis of student distribution across institutions, levels, and programs
//               </CardDescription>
//             </CardHeader>
//           </Card>
//         </div>

//         {/* Overall Program Stats Cards */}
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
//           <Card className="shadow-lg bg-gradient-to-br from-blue-50 to-blue-100">
//             <CardContent className="p-6 text-center">
//               <div className="p-4 bg-blue-600 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
...
//               <div className="text-3xl font-bold text-blue-900">{getTotalStudents().toLocaleString()}</div>
//               <div className="text-sm text-blue-700 font-semibold mt-2">Total Students</div>
//               <div className="text-xs text-blue-600 mt-2">Across all colleges</div>
//             </CardContent>
//           </Card>
...
//               </div>
//               <div className="text-3xl font-bold text-green-900">{getTotalPrograms()}</div>
//               <div className="text-sm text-green-700 font-semibold mt-2">Total Programs</div>
//               <div className="text-xs text-green-600 mt-2">Across all colleges</div>
//             </CardContent>
//           </Card>
...
//                 Student Distribution by Institution/Faculty
//               </CardTitle>
//               <CardDescription>Overall student distribution across all faculties and institutes</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <BarChart data={getAllInstitutionsData()}>
//                   <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                   <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 12, fontWeight: 600 }} angle={-45} textAnchor="end" height={80} />
//                   <YAxis tick={{ fill: '#64748b', fontSize: 11 }} />
//                   <Tooltip 
...
//                     dataKey="value"
//                   >
//                     {getAllLevelsData().map((entry, index) => (
//                       <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
//                     ))}
//                   </Pie>
//                   <Tooltip formatter={(value, name, props) => [
...
//                 Top Programs by Enrollment
//               </CardTitle>
//               <CardDescription>Most popular programs across all colleges</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <BarChart 
//                   data={getTopProgramsData()} 
//                   layout="vertical"
//                   margin={{ left: 150, right: 20 }}
//                 >
//                   <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
...
//                   <Tooltip 
//                     contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px' }}
//                     formatter={(value, name) => [value, name === 'value' ? 'Students' : 'Colleges']}
//                   />
//                   <Bar dataKey="value" fill="#8b5cf6" name="Total Students" radius={[0, 8, 8, 0]} />
//                 </BarChart>
//               </ResponsiveContainer>
//             </CardContent>
//           </Card>
...
//               <CardTitle className="flex items-center text-xl">
//                 <School className="h-5 w-5 mr-2 text-orange-600" />
//                 Institution-Level Combinations
//               </CardTitle>
//               <CardDescription>Most common institution-level combinations</CardDescription>
//             </CardHeader>
//             <CardContent>
//               <ResponsiveContainer width="100%" height={400}>
//                 <BarChart data={getInstitutionLevelCombinationData()}>
//                   <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                   <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} angle={-45} textAnchor="end" height={100} />
//                   <YAxis tick={{ fill: '#64748b', fontSize: 11 }} />
//                   <Tooltip 
...
//           <CardContent>
//             <ResponsiveContainer width="100%" height={400}>
//               <ComposedChart data={getCollegeProgramStats().slice(0, 15)}>
//                 <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
//                 <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} angle={-45} textAnchor="end" height={80} />
...
//                 />
//                 <Legend />
//                 <Bar yAxisId="left" dataKey="programs" fill="#3b82f6" name="Number of Programs" radius={[8, 8, 0, 0]} />
//                 <Line yAxisId="right" type="monotone" dataKey="avgStudentsPerProgram" stroke="#ef4444" name="Avg Students per Program" strokeWidth={2} />
//               </ComposedChart>
//             </ResponsiveContainer>
//           </CardContent>
//         </Card>
...
export default function AdminDashboardForProgress() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (reportsResponse.ok) {
        ...
        const sortedReports = (reportsData.data || []).sort((a: ProgressReport, b: ProgressReport) => 
  ...
  const handleExportAllPDF = async () => {
    try {
      ...
      pdf.setFontSize(20);
      pdf.setFont('helvetica', 'bold');
      pdf.text('ALL COLLEGES PROGRESS REPORT', 105, 15, { align: 'center' });
      
      pdf.setFontSize(12);
      pdf.text('Comprehensive Institutional Performance Analysis', 105, 22, { align: 'center' });
      ...
      pdf.setFontSize(16);
      pdf.setFont('helvetica', 'bold');
      pdf.text('OVERALL SUMMARY STATISTICS', 14, currentY);
      currentY += 10;
      ...
      const collegeSummaryData = reports.map((report, index) => {
        const analysis = getCollegeAnalysis(report);
        return [
          (index + 1).toString(),
          report.collegeName || 'N/A',
          report.academicYear || 'N/A',
          (report.totalStudents || 0).toLocaleString(),
          (report.programs?.length || 0).toString(),
          `${analysis.budgetUtilization}%`,
          `${analysis.studentClassroomRatio}:1`,
          report.buildingStatus || 'N/A',
      ...
      // Add each college's detailed report
      for (let i = 0; i < reports.length; i++) {
        const report = reports[i];
        const analysis = getCollegeAnalysis(report);

        // Add new page for each college after the first one
        if (i > 0) {
          pdf.addPage();
          currentY = 20;
        }
        ...
        pdf.text(`COLLEGE ${i + 1}: ${report.collegeName}`, 14, currentY);
        currentY += 8;
        ...
        const collegeMetrics = [
          ['Total Students', (report.totalStudents || 0).toLocaleString()],
          ['Total Programs', (report.programs?.length || 0).toString()],
          ['Budget Utilization', `${analysis.budgetUtilization}%`],
          ['Student:Classroom Ratio', `${analysis.studentClassroomRatio}:1`],
          ['Student:Lab Ratio', `${analysis.studentLabRatio}:1`],
          ['Graduation Rate', `${analysis.graduationRate}%`],
          ['Classrooms', (report.classroomCount || 0).toString()],
          ['Labs', (report.labCount || 0).toString()],
          ['Library Books', (report.libraryBooks || 0).toLocaleString()]
        ];
        ...
        if (report.programs && report.programs.length > 0) {
          pdf.setFontSize(12);
          pdf.setFont('helvetica', 'bold');
          pdf.text('PROGRAM SUMMARY', 14, currentY);
          currentY += 6;

          const programSummary = report.programs.slice(0, 5).map(program => [
            program.programName?.substring(0, 30) + (program.programName && program.programName.length > 30 ? '...' : '') || 'N/A',
            program.institution || 'N/A',
            program.level || 'N/A',
            (program.totalStudents || 0).toString(),
            `${program.passPercentage || 0}%`
          ...
          pdf.autoTable({
            startY: currentY,
            head: [['Program', 'Institution', 'Level', 'Students', 'Pass %']],
            body: programSummary,
            theme: 'grid',
            headStyles: { fillColor: [245, 158, 11], textColor: 255, fontStyle: 'bold' },
          ...
          if (report.programs.length > 5) {
            pdf.setFontSize(8);
            pdf.setFont('helvetica', 'italic');
            pdf.text(`... and ${report.programs.length - 5} more programs`, 14, currentY);
            currentY += 5;
          }
        }
        ...
        if (report.financialStatus) {
          ...
          const financial = report.financialStatus;
          const financialData = [
            ['Total Budget', formatCurrency(financial.totalAnnualBudget)],
            ['Total Expenditure', formatCurrency(financial.totalActualExpenditure)],
            ['Total Revenue', formatCurrency(financial.totalRevenueGenerated)],
            ['Utilization Rate', `${analysis.budgetUtilization}%`]
          ];
        ...
        pdf.setFontSize(12);
        pdf.setFont('helvetica', 'bold');
        pdf.text('PERFORMANCE ASSESSMENT', 14, currentY);
        currentY += 6;

        const performanceRating = getPerformanceRating(analysis);
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Overall Rating: ${performanceRating}`, 14, currentY);
        currentY += 5;
        ...
        // Check if we're approaching page end
        if (currentY > 250 && i < reports.length - 1) {
          // Will add new page in next iteration
        }
      }
      ...
      // Save the PDF
      pdf.save(`All_Colleges_Progress_Reports_${new Date().toISOString().split('T')[0]}.pdf`);
      toast.success('All colleges PDF report exported successfully');
    } catch (error) {
      console.error('Error exporting all colleges PDF:', error);
      toast.error('Failed to export all colleges PDF');
    }
  };
...
const formatCurrency = (amount: number): string => {
  // Helper: Convert English digits to Nepali digits
  const toNepaliNumber = (num: string): string => {
    const nepaliDigits = ['०', '१', '२', '३', '४', '५', '६', '७', '८', '९'];
    return num.replace(/\d/g, (digit) => nepaliDigits[parseInt(digit)]);
  };

  let formatted = '';

  if (amount >= 1_00_00_000) { // 1 crore
    formatted = `${(amount / 1_00_00_000).toFixed(2)} करोड`;
  } else if (amount >= 1_00_000) { // 1 lakh
    formatted = `${(amount / 1_00_000).toFixed(2)} लाख`;
  } else if (amount >= 1_000) { // 1 thousand
    formatted = `${(amount / 1_000).toFixed(2)} हजार`;
  } else {
    formatted = amount.toFixed(2);
  }

  // Convert to Nepali digits and format output
  return `रु ${toNepaliNumber(formatted)}`;
};
            ...
            <div className="flex items-center justify-between p-4 border-t">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-700">
                  Page {currentPage} of {totalPages} ({filteredReports.length} total colleges)
                </span>
