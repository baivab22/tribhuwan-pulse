...
function Header() {
  ...
  const progressFormIndex = mainNavigationItems.findIndex((item) => item.path === '/progress-form');
  const mainItemsBeforeCampus =
    progressFormIndex === -1
      ? mainNavigationItems
      : mainNavigationItems.slice(0, progressFormIndex);
  const mainItemsAfterCampus =
    progressFormIndex === -1
      ? []
      : mainNavigationItems.slice(progressFormIndex);
    ...
    >
      ...
      <div className="hidden lg:flex items-center space-x-3">
        <Select
          ...
          onValueChange={(v) => setLanguage(v as 'en' | 'ne')}
        >
                      ...
                      {feedbackItems.map((item) => {
                        ...
                        return (
                          <Link 
                            ...
                            onClick={() => setFeedbackMenuOpen(false)}
                          >
                            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-800 hover:bg-white hover:text-black transition-colors duration-150">
                              <IconComponent className="w-4 h-4" />
                              {item.label}
                            </button>
                          </Link>
        ...
        <div className="relative h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-6">
            <div className="max-w-sm mx-auto space-y-2">
              {/* Main Navigation */}
              {mainItemsBeforeCampus.map((item, index) => {
                const IconComponent = item.icon;
                const isActive = isActivePath(item.path);
                return (
                  <Link key={item.path} to={item.path}>
              ...
              {campusFormItems.map((item, index) => {
                const IconComponent = item.icon;
                const delayIndex = mainItemsBeforeCampus.length + index;
                const isActive = isActivePath(item.path);
                return (
                  <Link key={item.path} to={item.path}>
                    <div
              ...
              {mainItemsAfterCampus.map((item, index) => {
                const IconComponent = item.icon;
                const isActive = isActivePath(item.path);
                const delayIndex = mainItemsBeforeCampus.length + campusFormItems.length + index;
                return (
                  <Link key={item.path} to={item.path}>
                    <div
                      className={`
